...
#define SKIP_THRESHOLD           6 // Number of Pokémon to check are unseen before triggering a skip
        ...
        if (temp_isHoennDex)
        {
            ...
            for (i = 0, r5 = 0; i < temp_dexCount; i++)
            {
                ...
                if (!monSeen && i < temp_dexCount - 1 - SKIP_THRESHOLD)
                {
                    ...
                    {
                        if (GetSetPokedexFlag(RegionalToNationalOrder(i + j), FLAG_GET_SEEN))
                            break;
                    }
        ...
        else
        {
            ...
            for (u32 j = 0; evolutions[i].params != NULL && evolutions[i].params[j].condition != CONDITIONS_END; j++)
            {
                ...
                case IF_NOT_REGION:
                {
                    ...
                    case REGION_EXPANSION_1: StringAppend(gStringVar4, COMPOUND_STRING("Mauve")); break;
                    }
                    break;
                }
                // Gen 8
