    ...
    case EFFECT_PERISH_SONG:
        if (!isBattle1v1)
        {
            if (CountUsablePartyMons(battlerAtk) == 0
              ...
              && (aiData->abilities[GetPartnerBattler(battlerAtk)] != ABILITY_SOUNDPROOF || aiData->abilities[GetPartnerBattler(battlerAtk)] != ABILITY_GRIM_FAIRYTALE || !IsBattlerAlive(GetPartnerBattler(battlerAtk))))
            {
                ADJUST_SCORE(-10); //Don't wipe your team if you're going to lose
            }
            else if ((!IsBattlerAlive(GetBattlerLeftFoe(battlerAtk)) || aiData->abilities[GetBattlerLeftFoe(battlerAtk)] == ABILITY_SOUNDPROOF || aiData->abilities[GetBattlerLeftFoe(battlerAtk)] == ABILITY_GRIM_FAIRYTALE
              || gBattleMons[GetBattlerLeftFoe(battlerAtk)].volatiles.perishSong)
              && (!IsBattlerAlive(GetBattlerRightFoe(battlerAtk)) || aiData->abilities[GetBattlerRightFoe(battlerAtk)] == ABILITY_SOUNDPROOF || aiData->abilities[GetBattlerRightFoe(battlerAtk)] == ABILITY_GRIM_FAIRYTALE
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE)
            break;
        else if (aiData->holdEffects[battlerAtk] != HOLD_EFFECT_SILPH_SCOPE)
            break;
