...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_WEATHER_DAMAGE:
            ...
            {
                gBattleScripting.battler = battler;
                gBattleMoveDamage = -1 * max(1, GetNonDynamaxMaxHP(battler) / 12);
        ...
        case ENDTURN_INGRAIN:  // ingrain
            if ((gStatuses3[battler] & STATUS3_ROOTED)
             ...
             && IsBattlerAlive(battler))
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 12);
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_RAIN_DISH:
                 ...
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    ...
                    gBattleMoveDamage = GetNonDynamaxMaxHP(battler) / (gLastUsedAbility == ABILITY_RAIN_DISH ? 12 : 8);
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    effect++;
