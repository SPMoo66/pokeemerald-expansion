...
bool32 ShouldDefiantCompetitiveActivate(enum BattlerId battler, enum Ability ability)
{
    ...
    case ABILITY_KEEN_EYE:
        if (CompareStat(battler, STAT_EVASION, MAX_STAT_STAGE, CMP_EQUAL, ability))
            return FALSE;
        break;
    default:
        return FALSE;
...
bool32 CanAbilityAbsorbMove(struct DamageContext *ctx)
{
    ...
    case ABILITY_MOTOR_DRIVE:
        if (ctx->moveType == TYPE_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilities[ctx->battlerDef], STAT_SPEED, 2);
        break;
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SNOW_WARNING:
            if (!shouldAbilityTrigger)
                break;
            {
                u32 weather = (GetConfig(B_SNOW_WARNING) >= GEN_9 ? BATTLE_WEATHER_HAIL : BATTLE_WEATHER_HAIL);
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_RAIN_DISH:
                if (IsBattlerWeatherAffected(GetBattlerHoldEffect(battler), GetWeather(), B_WEATHER_RAIN)
                 ...
                 && !gBattleMons[battler].volatiles.healBlock)
                {
                    s32 healAmount = gLastUsedAbility == ABILITY_RAIN_DISH ? 12 : 8;
    ...
    case EFFECT_GYRO_BALL:
        {
            ...
            {
                basePower = ((30 * GetBattlerTotalSpeedStat(battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef])) / attackerSpeed) + 1;
                if (basePower > 150)
                    basePower = 150;
            }
            break;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_FLASH_FIRE:
        ...
        break;
    case ABILITY_SWARM:
        if (moveType == TYPE_BUG && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP * 2 / 5))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_TORRENT:
        if (moveType == TYPE_WATER && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP * 2 / 5))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_BLAZE:
        if (moveType == TYPE_FIRE && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP * 2 / 5))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_OVERGROW:
        if (moveType == TYPE_GRASS && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP * 2 / 5))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_PLUS:
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE)
	 ...
	 && usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE)
	 && GetAttackerWeather(ctx->holdEffects[ctx->battlerAtk], ctx->abilities[ctx->battlerAtk], ctx->weather) & B_WEATHER_HAIL
	 && usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline u32 IsBattlerLeekAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
    if (holdEffect == HOLD_EFFECT_LEEK)
    {
        return GET_BASE_SPECIES_ID(gBattleMons[battler].species) == SPECIES_FARFETCHD
            || gBattleMons[battler].species == SPECIES_MADAME
...
u32 GetTotalAccuracy(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, enum Ability atkAbility, enum Ability defAbility, enum HoldEffect atkHoldEffect, enum HoldEffect defHoldEffect)
{
    ...
    case ABILITY_VICTORY_STAR:
        if (IsBattlerAlive(atkAlly))
            calc = (calc * 120) / 100; // 2.1 ally's victory star boost
        break;
    default:
        break;
    }
