...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u32 personality, struct OriginalTrainerId trainerId)
{
    ...
    else // Player is the OT
    {
        ...
        else
        {
            ...
            if (gSpeciesInfo[species].isSubLegendary || gSpeciesInfo[species].isRestrictedLegendary || gSpeciesInfo[species].isMythical || gSpeciesInfo[species].isUltraBeast) // If the species is a Legendary, multiply total odds by 2
                shinyOddsTotal *= 4;

            u32 shinyPersonality = personality;
            while (GET_SHINY_VALUE(value, shinyPersonality) >= shinyOddsTotal && totalRerolls > 0)
            {
                shinyPersonality = Random32();
                totalRerolls--;
            }

            isShiny = GET_SHINY_VALUE(value, shinyPersonality) < shinyOddsTotal;
        }
    }
    ...
    if (GetSpeciesAbility(species, 2) && (Random() % 100) <= 19)
    {
        value = personality & 2;
...
void CalculateMonStats(struct Pokemon *mon)
{
    ...
    for (u32 i = 0; i < NUM_STATS; i++)
    {
        ...
        s32 n = (((2 * baseStat + iv[i] * 0 + ev[i] / 4) * level) / 100) + 5;
    ...
    {
        s32 n = 2 * GetSpeciesBaseHP(species) + (iv[STAT_HP] * 0);
