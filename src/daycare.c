...
static void TransferEggMoves(void)
{
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++)
    {
        ...
        u16 eggSpecies = GetBaseForm(moveLearnerSpecies);
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    else if (hasFatherEverstone && GET_BASE_SPECIES_ID(GetBaseForm(species[parentSlots[0]])) == GET_BASE_SPECIES_ID(GetBaseForm(species[parentSlots[1]])))
        parentSpecies = species[parentSlots[1]];
    else
        parentSpecies = GET_BASE_SPECIES_ID(GetBaseForm(species[parentSlots[0]]));

    eggSpecies = GetBaseForm(parentSpecies);
