    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gTasks[taskId].tMenuType)
        {
            ...
            default:
                ...
                AddTextPrinterParameterized3(4, FONT_NORMAL, GetStringRightAlignXOffset(FONT_NORMAL, gText_EmeralbodyVersionNumber, 0xD0), 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_EmeralbodyVersionNumber);
                PutWindowTilemap(0);
                PutWindowTilemap(1);
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_SMOLIV);
...
{
    return CreateMonPicSprite_Affine(SPECIES_SMOLIV, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
{
    FillBgTilemapBufferRect(bg, BIRCH_DLG_BASE_TILE_NUM +  0, x-2,       y-1, 1,       1, palNum);
    ...
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(BIRCH_DLG_BASE_TILE_NUM + 0), x-2,       y+height, 1,       1, palNum);
