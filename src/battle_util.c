...
bool32 ShouldDefiantCompetitiveActivate(enum BattlerId battler, enum Ability ability)
{
    ...
    if (ability != ABILITY_DEFIANT && ability != ABILITY_COMPETITIVE && ability != ABILITY_KEEN_EYE)
        return FALSE;
...
bool32 CanAbilityAbsorbMove(struct BattleContext *ctx)
{
    ...
    case ABILITY_MOTOR_DRIVE:
        if (ctx->moveType == TYPE_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_SPEED, 2);
        break;
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SNOW_WARNING:
            if (!shouldAbilityTrigger)
                break;
            {
                u32 weather = (GetConfig(CONFIG_SNOW_WARNING) >= GEN_9 ? BATTLE_WEATHER_HAIL : BATTLE_WEATHER_HAIL);
    ...
    case EFFECT_GYRO_BALL:
        {
            ...
            {
                basePower = ((30 * GetBattlerTotalSpeedStat(battlerDef, ctx->abilityDef, ctx->holdEffectDef)) / attackerSpeed) + 1;
                if (basePower > 150)
                    basePower = 150;
            }
            break;
