...
*/
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SLOW_START:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gDisableStructs[battler].slowStartTimer = gBattleTurnCounter + 2;
        ...
        case ABILITY_SNOW_WARNING:
            if (GetGenConfig(GEN_SNOW_WARNING) >= GEN_9 && TryChangeBattleWeather(battler, BATTLE_WEATHER_HAIL, gLastUsedAbility))
            {
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_RAIN_DISH:
                 ...
                 && !gBattleMons[battler].volatiles.healBlock)
                {
                    ...
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / (gLastUsedAbility == ABILITY_RAIN_DISH ? 12 : 8);
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
                    gBattleStruct->moveDamage[battler] *= -1;
                    effect++;
    ...
    case EFFECT_GYRO_BALL:
        {
            ...
            {
                basePower = ((30 * GetBattlerTotalSpeedStat(battlerDef)) / attackerSpeed) + 1;
                if (basePower > 150)
                    basePower = 150;
            }
            break;
