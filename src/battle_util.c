...
void PrepareStringBattle(u16 stringId, u32 battler)
{
    ...
    // Check Defiant and Competitive stat raise whenever a stat is lowered.
              ...
              && ((targetAbility == ABILITY_DEFIANT && CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
                 ...
                 || (targetAbility == ABILITY_KEEN_EYE && CompareStat(gBattlerTarget, STAT_ACC, MAX_STAT_STAGE, CMP_LESS_THAN)))
...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_WEATHER_DAMAGE:
            ...
            {
                gBattleScripting.battler = battler;
                gBattleStruct->moveDamage[battler] = -1 * max(1, GetNonDynamaxMaxHP(battler) / 12);
        ...
        case ENDTURN_INGRAIN:  // ingrain
            ...
            {
                gBattleStruct->moveDamage[battler] = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 12);
...
static inline u32 CalcMoveBasePower(struct DamageCalculationData *damageCalcData, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_GYRO_BALL:
        basePower = ((30 * GetBattlerTotalSpeedStat(battlerDef)) / GetBattlerTotalSpeedStat(battlerAtk)) + 1;
        if (basePower > 150)
            basePower = 150;
        break;
