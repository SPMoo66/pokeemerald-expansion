...
//    enum Species hiddenSpecies[NUM_HIDDEN_MONS_ENCOUNTER_SLOTS];
    enum Species fishingSpecies[NUM_FISHING_MONS_ENCOUNTER_SLOTS];
    u8 cursorRow;
    u8 cursorCol;
    u8 environment;
    u8 potential;
            ...
            else if (IsMapTypeIndoors(currMapType))
            {
                if (MetatileBehavior_IsTallGrass(metatileBehaviour)) //Grass in cave
                    ...
                    fldEffId = FLDEFF_SHAKING_LONG_GRASS;
                         ...
                         || MetatileBehavior_IsSandGrass(metatileBehaviour))
...
static u8 DexNavTryGenerateMonLevel(enum Species species, enum EncounterType environment)
{
    ...
    if (Random() % 100 < 5)
        levelBonus += 1; //5% chance of having a +1 level
...
//        break;
    case 3: //fishing
        for (i = 0; i < NUM_FISHING_MONS_ENCOUNTER_SLOTS; i++)
        {
            if (sDexNavUiDataPtr->fishingSpecies[i] == species)
                return TRUE;
        }
        break;
    default:
...
static void Task_DexNavMain(u8 taskId)
{
    ...
    else if (JOY_NEW(R_BUTTON))
    {
        ...
        if (species != SPECIES_NONE)
        {            
            if (((sDexNavUiDataPtr->environment << 14) | species) == VarGet(VAR_DEXNAV_SPECIES)) { //This checks if a species is already registered
                PlaySE(SE_BOO);
                PrintSearchableSpecies(SPECIES_NONE);
                VarSet(VAR_DEXNAV_SPECIES, SPECIES_NONE); //Unregisters a species
            }
            else if (sDexNavUiDataPtr->cursorRow == ROW_FISHING) // Don't want the player to select a fishing mon
            {
    ...
    else if (JOY_NEW(A_BUTTON))
    {
        ...
        else
        {
            ...
            gSpecialVar_0x8002 = (sDexNavUiDataPtr->cursorRow == ROW_FISHING) ? TRUE : FALSE;
            PlaySE(SE_DEX_SEARCH);
            BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 16, RGB_BLACK);
