...
static void BlitBitmapToPartyWindow_Equal(u8, u8, u8, u8, u8, bool8); //Custom party menu
...
static u16 PartyMenuButtonHandler(s8 *slotPtr)
{
    ...
    if (JOY_NEW(R_BUTTON) && CheckBagHasItem(ITEM_POKEMON_BOX_LINK, 1))
        return R_BUTTON;
    ...
    {// PARTY_SIZE + 1 is Cancel, PARTY_SIZE is Confirm
        ...
        case MENU_DIR_DOWN:
            ...
            else
            {
                ...
                else if(*slotPtr+2 < gPartiesCount[B_TRAINER_PLAYER])
                {
                    *slotPtr += 2;//(*slotPtr)++;
                }
				else
            ...
            break;
        case MENU_DIR_RIGHT:
            if (*slotPtr+1 < gPartiesCount[B_TRAINER_PLAYER])
                (*slotPtr)++;
            // else
            //     *slotPtr = sPartyMenuInternal->lastSelectedSlot;
            else if (*slotPtr == PARTY_SIZE + 1)
			...
			else if (*slotPtr == gPartiesCount[B_TRAINER_PLAYER] - 1)
			{
				*slotPtr = PARTY_SIZE + 1;
			}
            break;
...
static void Task_ReturnToChooseMonAfterText(u8 taskId)
{
    if (IsPartyMenuTextPrinterActive() != TRUE)
    {
        ...
        else
        {
            ...
            else if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(ITEM_POKEMON_BOX_LINK, 1))
                DisplayPartyMenuStdMessage(PARTY_MSG_CHOOSE_MON_R);
            else
                DisplayPartyMenuStdMessage(PARTY_MSG_CHOOSE_MON);
            gTasks[taskId].func = Task_HandleChooseMonInput;
        }
...
static void Task_PartyMenuModifyHP(u8 taskId)
{
    ...
    DisplayPartyPokemonMaxHPCheck(&party[partySlot], &sPartyMenuBoxes[partySlot], 1);
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        for (j = 0; j != FIELD_MOVES_COUNT; j++)
        {
            ...
            {
                if (FieldMove_GetMoveId(j) != MOVE_FLY) // If Mon already knows FLY, prevent it from being added to action list
    ...
    if (sPartyMenuInternal->numActions < 5 && FlagGet(FLAG_BADGE02_GET)) // Once Badge 2 is obtained add fly to mon menu
...
static void FinishTwoMonAction(u8 taskId)
{
    ...
    AnimatePartySlot(gPartyMenu.slotId2, 1);
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(ITEM_POKEMON_BOX_LINK, 1))
        DisplayPartyMenuStdMessage(PARTY_MSG_CHOOSE_MON_R);
    else
        DisplayPartyMenuStdMessage(PARTY_MSG_CHOOSE_MON);
    gTasks[taskId].func = Task_HandleChooseMonInput;
...
{
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(ITEM_POKEMON_BOX_LINK, 1))
        InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, TRUE, PARTY_MSG_CHOOSE_MON_R, Task_HandleChooseMonInput, CB2_ReturnToFieldWithOpenMenu);
    else
...
{
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(ITEM_POKEMON_BOX_LINK, 1))
        InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON_R, Task_HandleChooseMonInput, CB2_ReturnToFieldWithOpenMenu);
    else
...
{
    if (BW_SUMMARY_SCREEN)
        ShowPokemonSummaryScreen_BW(SUMMARY_MODE_SELECT_MOVE, gParties[B_TRAINER_PLAYER], gSpecialVar_0x8004, gPartiesCount[B_TRAINER_PLAYER] - 1, CB2_ReturnToField);
    else
